({
    loadIntersectionDetails : function(component){
        var action = component.get("c.getAccConRel");
        action.setParams({"relationId": component.get("v.recordId")});
        
        //Configure response handler
        action.setCallback(this, function(response){
			var state = response.getState();
            if(component.isValid() && state === "SUCCESS") {
            	console.log('###AccConRel: ' + response.getReturnValue().Id)
                component.set("v.accConRel", response.getReturnValue());
                
            } else {
                console.log('Problem getting relation, response state: ' + state);
            }                           
        });
	...
	loadAccountDetails : function(component) {
        ...
        action.setCallback(this, function(response) {
            var state = response.getState();
            if(component.isValid() && state === "SUCCESS") {
            	console.log('###account: ' + response.getReturnValue().Id)
	...
	validateEventForm: function(component) {
        ...
        var account = component.get("v.accConRel.AccountId");
        var contact = component.get("v.accConRel.ContactId");
        console.log('###AccId: '+ account);
        console.log('###ConId: '+ contact);
        if($A.util.isEmpty(account) || $A.util.isEmpty(contact)) {
            validEvent = false;
            console.log("Quick action context doesn't have a valid account/contact.");
        }
	...
	saveEvent: function(component, event){
		var accId = component.get("v.accConRel.AccountId");
		var conId = component.get("v.accConRel.ContactId");
		...
		saveEventAction.setParams({
			...
			"contactId": component.get("v.accConRel.ContactId"),
			"accountId": component.get("v.accConRel.AccountId"),
